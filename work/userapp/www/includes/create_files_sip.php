...
function _create_sip_beroPBX_codecs ($ba, $trunkid) {
...
function create_sip_beroPBX ($ba, $ami) {

	$fn = BAF_APP_AST_CFG . '/sip_beroPBX.conf';
	...
	while ($entry = $ba->fetch_array($query)) {
		...
		$secs .=	"[" . $name . "]\n" .
				...
				"disallow = all\n" .
				"allow = " . _create_sip_beroPBX_codecs($ba, $entry['id']) . "\n" .
	...
	$cont =	"; Generated by beroPBX 'create_files_sip.php'\n\n" .
		"[general](+)\n" .
		(!empty($regs) ? $regs . "\n" : '') .
		(!empty($secs) ? $secs : '');
...
//		"type = peer\n" .
		"\n" .
		"#include " . BAF_APP_AST_CFG . "/sip_beroPBX.conf\n";
